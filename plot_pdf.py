...
def export_cu_enaks_konus_pdf(
    ...
    A4 landscape PDF:
      ...
      - Bottom fixed 0.3 cm above title block; top lowered so labels inside border
      - Legend: boreholes (dedup), 4 columns, vertically centered to title block,
                with a legend title, and a color-neutral marker key (● Enaks, ▲ Konus)
      - Title block layout:
          Row 1: 2/3 Rapport Nr., 1/3 Figur Nr.
          Row 2: 3 equal (Tegn, Kontr, Godkj)
          Row 3: full width (Dato)
          Bottom: Logo (taller), bottom-right, no stretching
      - Konus marker = '^', Enaks marker = 'o', same colour for same BH
    """
    if title_info is None:
        title_info = {}
    rapport_nr = title_info.get("rapport_nr", "XX")
    ...
    # ---- TITLE BLOCK ----
    tb_width  = inner_w * 0.30
    tb_height = inner_h * (0.26 - 0.04)  # footer_h_frac - tiny breathing
    tb_left   = inner_left + inner_w - tb_width
    tb_bottom = inner_bottom  # overlap page border
    ...
    # Tighter rows: grid ≈ 45%, logo ≈ 55% (from your latest tweak)
    logo_h_frac = 0.55
    grid_height = tb_height * (1 - logo_h_frac)
    row_h = grid_height / 3.0
    r1_top = tb_bottom + tb_height
    ...
    # ===== Charts =====
    charts_bottom = (tb_bottom + tb_height) + (0.3/2.54)/fig_h  # bottom fixed
    charts_top = inner_top - 0.07                               # lowered so titles/ticks sit inside border
    charts_height = max(0.05, charts_top - charts_bottom)

    # Axes positions (leave inner margins so labels/ticks stay inside border)
    left_ax  = fig.add_axes([inner_left + inner_w*0.06, charts_bottom, inner_w*0.40, charts_height])
    right_ax = fig.add_axes([inner_left + inner_w*0.54, charts_bottom, inner_w*0.38, charts_height])
    ...
    # ===== Legend (boreholes, dedup) =====
    # Build dedup labels from union of BH present (terrain Z from whichever series has it)
    bh_to_Z = {}
    ...
    for bh in all_bhs:
        if bh in bh_to_Z:
            handle = plt.Line2D([], [], linestyle='', marker='s', markersize=8, color=bh_color[bh])
            legend_handles.append(handle)
            legend_labels.append(f"{bh}, {bh_to_Z[bh]:.1f} m")

    legend_title = "Sammenstilling udrenert skjærstyrke (enaks og konus)"
    ...
    if legend_handles:
        ...
        # Ensure the legend title is readable
        if lg.get_title():
            lg.get_title().set_fontsize(9)
            lg.get_title().set_weight('bold')

              # ---- Colour-neutral marker key (● Enaks, ▲ Konus) ----
        key_handles = [
        ...
        fig.legend(
            key_handles, [h.get_label() for h in key_handles],
            loc='upper left',
            bbox_to_anchor=(legend_x0, legend_y0 + legend_h + 0.005, 0.2, 0.03),
            bbox_transform=fig.transFigure,
